...
# app = Flask(__name__)
app.secret_key = "dlskjfsalkfj80294378"
...
def contact():
    if request.method == "POST":
        ...
        x.commit()
        x.close()
        return render_template('thanks.htm', name= n)
...
def messages():
    ...
    m = cursor.fetchall()
    x.close()
    return render_template('messages.htm', messages = m)
...
def profile():
        ...
        x.close()
        if user:
            return render_template('profile.htm', username = user[1])
